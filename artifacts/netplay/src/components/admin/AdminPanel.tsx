...
const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const [logoMode, setLogoMode] = useState<LogoScanMode>('all');
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        ...
        <aside className="w-full md:w-72 border-b md:border-b-0 md:border-r border-white/5 bg-black/20 flex flex-col shrink-0 overflow-hidden">
          <div className="flex flex-row md:flex-col p-2 md:p-6 space-y-0 md:space-y-2 overflow-x-auto md:overflow-y-auto scrollbar-hide">
            ...
            {[
              ...
              { id: 'episode-settings', label: 'Padrões de Episódio', icon: Zap },
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-6 md:p-8 scrollbar-hide">
            ...
            ) : activeTab === 'episode-settings' ? (
              <AdminEpisodeSettingsTab />
            ) : activeTab === 'quente' ? (
                    ...
                    <div>
                      ...
                      <p className="text-[9px] text-gray-500 mt-1 px-1">
                        {((editingMovie.videoUrl || editingMovie.videoUrl2 || '').startsWith('terabox-folder://') && !(editingMovie.videoUrl || editingMovie.videoUrl2 || '').startsWith('terabox-folder-v2://') && !(editingMovie.videoUrl || editingMovie.videoUrl2 || '').startsWith('terabox-folder-v3://'))
                          ? `🎯 Vídeo Automático (API 1 Pro): testa cada qualidade por até ${(editingMovie as any).qualityCascadeDelay ?? 10}s — 1080p → 720p → 480p → ... → se tudo falhar, muda para API 3.0 automaticamente.`
                      ...
                      <div className="mt-3 bg-white/5 border border-white/10 rounded-xl p-4 space-y-3">
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-[9px] text-gray-500 mt-0.5">Segundos que o player aguarda cada qualidade antes de tentar a próxima (e API 3.0 ao final)</p>
                          </div>
