...
export default function AdminTeraboxTab({ movies, onUpdateMovie, onAddMovie }: { movies: Movie[], onUpdateMovie: Function, onAddMovie: Function }) {
  ...
  const parseEpisodeName = (filename: string): string | null => {
    ...
    // Remove indicadores de qualidade/codec/fonte e tudo depois deles
        ...
        try {
          ...
          updated[seasonNum] = (updated[seasonNum] as any[]).map((file) => {
            ...
            if (!match && file.detectedEpisodeName) {
              ...
              // Primeiro tenta match exato pelo nome normalizado
              match = byName.get(normalizedDetected);
              if (!match) {
                // Tenta match parcial por sobreposição de palavras (mínimo 50%)
                let bestScore = 0;
            ...
            // 3ª tentativa: mesmo sem detectedEpisodeName, busca pelo nome do arquivo completo
            if (!match) {
