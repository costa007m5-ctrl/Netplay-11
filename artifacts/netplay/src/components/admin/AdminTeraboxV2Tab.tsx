...
export default function AdminTeraboxV2Tab({ movies, onUpdateMovie, onAddMovie }: { movies: Movie[], onUpdateMovie: Function, onAddMovie: Function }) {
  ...
  const handleTest = async () => {
    ...
    try {
      const res = await fetch(`/api/terabox-v2?url=${encodeURIComponent(testUrl)}`);
  ...
  const handleScanFolder = async () => {
    ...
    try {
      const res = await fetch(`/api/terabox-v2?url=${encodeURIComponent(folderUrl)}`);
      const data = await res.json();
      if (!res.ok) throw new Error(`${data.error}: ${data.details || ''}`);
  ...
  const handleScanAllSeasons = async () => {
    ...
    for (const sf of validFolders) {
      ...
      try {
        const res = await fetch(`/api/terabox-v2?url=${encodeURIComponent(sf.folderUrl)}`);
        const data = await res.json();
    ...
    try {
      for (const item of toSave) {
        ...
        const videoUrlToSave = dynamicLinkMode
          ? makeDynamicRefV2(folderUrl, item.imported_filename)
          : folderUrl;
  ...
  const getDirectLinkFromApi = async (url: string) => {
    const res = await fetch(`/api/terabox-v2?url=${encodeURIComponent(url)}`);
    const data = await res.json();
    if (!res.ok) throw new Error(`${data.error}: ${data.details || ''}`);
  ...
  const handleUpdateAll = async () => {
    if (!confirm(`Tem certeza que deseja processar ${teraboxMovies.length} filmes usando a API Terabox 2.0 (teraboxdl.site)? Esta ação irá converter os links de origem para links diretos.`)) return;
    ...
    <div className="max-w-4xl mx-auto py-8">
      <div className="mb-8">
        <h2 className="text-3xl font-black italic uppercase tracking-tighter flex items-center gap-3 text-cyan-400">
          <Database size={36} /> Administração API Terabox 2.0
        </h2>
        <p className="text-gray-400 text-sm mt-2">
          Integração automática para links Terabox e KingX usando a API Terabox 2.0 (teraboxdl.site).
        </p>
      </div>
      ...
      <div className="bg-gradient-to-br from-cyan-500/10 to-transparent border border-cyan-500/30 rounded-2xl p-6 mb-8">
        ...
        <p className="text-sm text-gray-400 mt-2">
          O servidor está configurado para utilizar a chave de API <code>X-API-KEY (TERABOX_V2_API_KEY)</code>.
          O endpoint local <code>/api/terabox-v2</code> processará os links automaticamente no player e ferramentas abaixo.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
          ...
          <p className="text-xs text-gray-400 mb-4">
             Converta links do KingX/Terabox no seu catálogo para os novos links diretos gerados pela API Terabox 2.0 (teraboxdl.site).
