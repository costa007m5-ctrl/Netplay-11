...
function EpgInfoModal({
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div className="relative bg-gradient-to-br from-[#1a1a2e] to-[#141414] p-5 flex flex-col items-center gap-3">
          <button
            onClick={onClose}
          ...
          <div className="w-20 h-20 rounded-2xl bg-black/40 border border-white/10 flex items-center justify-center overflow-hidden">
            {img && !imgErr ? (
              <img src={img} alt={getName(channel)} className="w-16 h-16 object-contain" onError={() => setImgErr(true)} />
            ) : (
              <Radio size={30} className="text-gray-600" />
            )}
          </div>
        ...
        <div className="p-5 space-y-4">
          {/* Live status */}
          <div className="flex items-center gap-2 bg-[#e8172c]/10 border border-[#e8172c]/20 rounded-xl px-3 py-2">
          ...
          {/* Current program */}
          {epg ? (
            <div className="space-y-3 bg-white/4 rounded-xl p-3 border border-white/5">
              <div className="flex items-center justify-between">
                <span className="text-[9px] font-black uppercase tracking-widest text-gray-500">Agora</span>
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[8px] text-gray-600 font-mono">{pct}%</span>
                </div>
              </div>
            </div>
          ) : (
            <div className="text-center py-3 text-gray-600 text-xs">Programação não disponível</div>
          )}
    ...
    <div className="flex items-center gap-0 pr-2 hover:bg-white/3 transition-colors group">
      ...
      <div className="flex-1 min-w-0 overflow-x-auto scrollbar-none flex items-center gap-2 py-2">
        ...
        {epg && (
          <button
            onClick={onInfo}
            className="shrink-0 text-left min-w-[70px]"
          >
...
// ─── Sidebar channel item (extracted to avoid hooks-in-map violation) ──────────
function SidebarChannelItem({
  ch, active, onSelect,
}: {
  ch: Channel; active: boolean; onSelect: () => void;
}) {
  const [imgErr, setImgErr] = useState(false);
  const img = getImage(ch);
  const cat = getCat(ch);
  return (
    <button
      ...
      className={`w-full flex items-center gap-3 px-2 py-2 rounded-xl text-left transition-all ${
        active ? 'bg-[#e8172c]/20 border border-[#e8172c]/30' : 'hover:bg-white/5 border border-transparent'
      }`}
    >
...
// ─── Custom channel player (iframe + controls in same container) ───────────────
function ChannelPlayerView({
  channel, allChannels, onClose, onPiP, onSwitch,
}: {
  channel: Channel;
  allChannels: Channel[];
  onClose: () => void;
  ...
  // Fullscreen on container (includes controls — fixes landscape issue)
  ...
  // Intercept hardware back button → PiP
  useEffect(() => {
  ...
  }, [goNext, goPrev, showSidebar, onClose]);

  const { data: epgData } = useEpg(channel.id);
  ...
  return (
    ...
    >
      ...
      {/* Controls overlay — always inside the same container so they survive fullscreen */}
      <AnimatePresence>
        {showControls && (
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            {/* Top bar */}
            <div className="absolute top-0 left-0 right-0 bg-gradient-to-b from-black/70 to-transparent pt-2 pb-8 px-3 flex items-center gap-2 pointer-events-auto">
              {/* Back → PiP */}
              <button
              ...
              <div className="flex-1 flex items-center gap-2 min-w-0 ml-1">
                {getImage(channel) && (
                  <img src={getImage(channel)} alt="" className="w-7 h-7 rounded-lg object-contain bg-black/40 p-0.5 shrink-0" />
                )}
                <div className="min-w-0">
                  <p className="text-white text-sm font-black truncate leading-none">{getName(channel)}</p>
                  {current && (
                    <p className="text-gray-400 text-[10px] truncate">{current.title}</p>
                  )}
                </div>
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                ...
                <button
                  ...
                  className={`w-9 h-9 rounded-xl backdrop-blur-xl border flex items-center justify-center transition-all ${
                    showSidebar ? 'bg-[#e8172c]/40 border-[#e8172c]/60 text-white' : 'bg-black/60 border-white/20 text-white hover:bg-white/10'
                  }`}
                >
                  <List size={15} />
                </button>
            ...
            {/* Side nav buttons (always visible in center of screen) */}
            <button
            ...
            {/* Bottom EPG bar */}
            {current && (
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/75 to-transparent pt-10 pb-3 px-4 pointer-events-auto">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <div className="flex-1 h-[3px] bg-white/15 rounded-full overflow-hidden">
                        <div className="h-full bg-[#e8172c] rounded-full" style={{ width: `${current.progress}%` }} />
                      </div>
                ...
                <p className="text-[8px] text-white/20 font-mono mt-1 select-none">← → trocar canal • L lista</p>
              </div>
            )}
          </motion.div>
        )}
          ...
          >
            <div className="flex-none p-3 border-b border-white/5">
              <div className="flex items-center gap-2 mb-3">
                <span className="w-2 h-2 bg-[#e8172c] rounded-full animate-pulse" />
...
function PiPPlayer({
  ...
  return (
    <motion.div
      ...
      className="fixed bottom-20 right-3 z-[7000] w-48 rounded-2xl overflow-hidden shadow-2xl border border-white/20 bg-black"
    >
      {/* iframe */}
  ...
  if (playingChannel) {
    return (
      <>
        ...
        />
        {/* PiP still visible if switching */}
      </>
    );
  }
    ...
    <div className="min-h-screen bg-[#111] text-white pb-24">
      ...
      {!loading && !error && (
        <>
          ...
          {/* Channel list — flat, no collapsible folders */}
          {search ? (
              ...
              {visibleCategories.map(cat => {
                ...
                return (
                  <div key={cat}>
                    ...
                    {/* All channels in this category — always visible */}
                    {chs.map(ch => (
                      <ChannelRow
                        key={ch.id}
                        ch={ch}
              ...
              {!activeCategory && (() => {
                ...
                return (
                  <div>
                    ...
                    {uncat.map(ch => (
                      <ChannelRow key={ch.id} ch={ch} epg={epgMap[ch.id] || null} onPlay={() => setPlayingChannel(ch)} onInfo={() => setInfoTarget(ch)} />
                    ))}
                  </div>
                );
              })()}
